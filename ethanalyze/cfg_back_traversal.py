    ...
    if state.gas is None or state.gas > 0:
        ...
        for p in state.bb.pred:
            ...
            for new_must_visit in minimize(new_must_visits):
                new_todo.append(TraversalState(p, new_gas, new_must_visit, new_cost, update_data(new_data, p)))
    return new_todo


def traverse_back(start_ins, initial_gas, initial_data, advance_data, update_data, finish_path, must_visits=[], predicate=lambda st,p:True):
    """
    ...
    for ins in start_ins:
        ...
        for must_visit in minimize(FrontierSet(mv) if mv is not FrontierSet else mv for mv in must_visits):
            ts = TraversalState(bb, gas, must_visit, 0, data)
            todo.put((ts.rank(), ts))
