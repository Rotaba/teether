...
class FrontierSet(object):
    ...
    def all(self):
        if self._frozenall is None:
            self._frozenall = frozenset(set(self._blocking_set.keys() + self._inhibiting_set.keys()) | self._frontier)
    ...
    def __repr__(self):
        return '{%s|%s}'%(','.join('%x'%i for i in self.frontier), ','.join('%x'%i for i in self.all-self.frontier))